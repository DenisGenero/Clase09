...
const register = async (req, res) => {
    ...
    const dbResponse = await registerNewUser({...req.body, password : password, image})
    if(dbResponse instanceof Error) return next(dbResponse)
    const tokenData = {
...
const login = async (req, res, next) => {
    ...
    const passwordMatch = await checkPassword(req.body.password, dbResponse[0].password) // devuelve un true o false
    if (passwordMatch){
        const user = {
            id: dbResponse[0].id,
